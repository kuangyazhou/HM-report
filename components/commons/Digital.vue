...
export default {
  ...
  watch: {
    number: function (val) {
      this.currentSaleAmount.update(val)
    }
  },
  data () {
    return {
  ...
  mounted () {
    this.initPlugin()
    this.currentSaleAmount = new this.Odometer({
      el: this.$el,
      value: 0,
      format: ',ddd',
      theme: 'train-station'
  ...
  methods: {
    initPlugin: function () {
      ...
      createFromHTML = function (html) {
        var el
        el = document && document.createElement('div')
        el.innerHTML = html
...
<style>
  ...
  .odometer.odometer-auto-theme .odometer-digit, .odometer.odometer-theme-train-station .odometer-digit {
    ...
    *display: inline;
    ...
    background-size: 100%;
    /* background-image: linear-gradient(to bottom, rgba(252, 252, 253, 0.27) 0%, rgba(252, 252, 253, 0.27) 35%, rgba(252, 252, 253, 0.1) 55%, rgba(252, 252, 253, 0.27) 55%, rgba(252, 252, 253, 0.27) 100%); */
    background-color: rgba(252, 252, 253, 0.27);
    padding: 0 0.15em;
    color: #fff;
  }
